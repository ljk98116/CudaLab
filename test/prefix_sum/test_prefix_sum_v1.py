...
epoch = 10000
# 100000000数据规模，迭代100次，算子时间5587us
def validate() -> bool:
    N = random.randint(1, 100)
    x = torch.randn((N)).cuda().contiguous()
    ref = torch.cumsum(x, 0).cpu()
    torch.cuda.synchronize()
    res = torch.ops.CudaLab.prefix_sum_v1_fp32_1d(x).cpu()
    torch.cuda.synchronize()
    ...
    try:
        ...
        print(f"validate prefix_sum_v1_fp32_1d failed at N:{N}, x:{x}, ref:{ref}, res:{res}")
        return False
    
    return True
    
def validate_prefix_sum_v1_fp32_1d():
    for i in range(epoch):
        if validate() is False:
            return False
    print("validate prefix_sum_v1_fp32_1d done")
    return True
...
def run_benchmark_prefix_sum_v1_fp32_1d():
    ...
    run_benchmark(
      torch.ops.CudaLab.prefix_sum_v1_fp32_1d,
      x,
      "fp32_1d",
      out
    )

def test_prefix_sum_v1():
    validate_prefix_sum_v1_fp32_1d()
    run_benchmark_prefix_sum_v1_fp32_1d()
